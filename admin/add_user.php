...
//On vérifie si tous les champs sont remplis
    if(empty($_POST['username']) && empty($_POST['email']) && empty($_POST['type']) && empty($_POST['password'])){
...
//On vérifie si l'utilisateur a entré un username
    ...
    //on vérifie que le champ username n'est pas déjà utilisé par un autre membre
      ...
      //on crypte le mot de passe avec la fonction password_hash()
        $req = $bdd->prepare("INSERT INTO users SET username='".$_POST['username']."', email='".$_POST['email']."',  type='".$_POST['type']."', password='".password_hash($_POST['password'], PASSWORD_DEFAULT)."'");
        $result = $req->execute(array($_POST['username'], $_POST['email'], $_POST['type'], $_POST['password']));
        if($result){
        ?><?php include("../includes/adduser.php")?>
    	<?php
