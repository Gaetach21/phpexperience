...
<html>
  ...
  <head>
    ...
    <title>phpexperience | Page de création des utilisateurs</title>
  </head>
...
if(isset($_POST['name'],$_POST['email'],$_POST['type'],$_POST['password'],$_POST['confpwd'])){
...
//On vérifie si tous les champs sont remplis
    if(empty($_POST['name']) && empty($_POST['email']) && empty($_POST['type']) && empty($_POST['password']) && empty($_POST['confpwd'])){
    ...
    //Si tous les champs sont vides
        ...
        echo "<span>Le champ type est obligatoire!</span><br>";
...
//On vérifie si l'utilisateur a entré un nom
    ...
    //le champ mot de passe est vide
        echo "<span>Le champ type est obligatoire!</span>";
    }
    ...
    //on vérifie que le champ name n'est pas déjà utilisé par un autre membre
      ...
      //on crypte le mot de passe avec la fonction password_hash()
        $req = $bdd->prepare("INSERT INTO utilisateurs SET name='".$_POST['name']."', email='".$_POST['email']."',  type='".$_POST['type']."', password='".password_hash($_POST['password'], PASSWORD_DEFAULT)."',date_inscription = NOW()");
        $result = $req->execute(array($_POST['name'], $_POST['email'], $_POST['type'], $_POST['password']));
        if($result){
        ?><?php include("../includes/adduser.php")?>
    	<?php
            echo "<div class='success'>
             <h3>L'utilisateur a été créée avec succés.</h3>
             <p>Cliquez <a href='home.php'>ici</a> pour retourner à la page admin</p>
       </div>";
        } 
    }
}
